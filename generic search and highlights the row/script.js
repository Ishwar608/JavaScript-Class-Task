...
async function getValue() {
    ...
    await new Promise((suceess, reject) => {
        if (url.status == 200) {
            ...
            reject(document.getElementById("tblbody").innerHTML = `<img src="https://aioseo.com/wp-content/uploads/2021/04/how-to-find-and-fix-404-errors-in-wordpress.png" alt=""/>`);
...
searchData = () => {
    ...
    for (const i of tr) {
        ...
        for (const j of filterData) {
            if (i.id == j.id) {
                document.getElementById(`${i.id}`).style.background = "yellow";
            }
